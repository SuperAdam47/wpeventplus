					...
					<div class="media-box <?php echo $catStr; ?>" data-columns="<?php echo $col; ?>">
						...
						<div class="media-box-content" style="background-color: #f5f5f5;">
							...
							if( isset($event->outside_reg) && $event->outside_reg == 'Y' ) {
								$permaLink = !empty( $event->external_site ) ? $event->external_site : EventPlus_Helpers_Event::permalink($company_options['evrplus_page_id']);
							} else {

								$recurr= isset( $recurr ) ? $recurr : false;

								$permaLink = EventPlus_Helpers_Event::permalink($company_options['evrplus_page_id']);
								$permaLink .= "?action=evrplusegister&event_id=".$event->id . ( ($recurr) ? '&recurr=' . $recurr : '' );
							} ?>

							<div class="media-box-more">
								<a href="<?php echo $permaLink; ?>">
									<?php _e('Read more', 'evrplus_language'); ?>
								</a>
							</div>
						</div>
					</div>
